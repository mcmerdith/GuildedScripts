...
def main():
    ...
    opt, args = getopt.getopt(sys.argv[1:], "fnh", [
                              "force", "novalidate", "help"])
    args = args[1:]
    ...
    if args is None or len(args) == 0:
        ...
        if arg is None or len(arg) == 0:
            args = list(filter(lambda file: not file.endswith(
                ".backup"), os.listdir(os.getcwd())))
        else:
            args = []
        ...
        if not os.path.exists(path) or not os.path.isfile(path):
            ...
            with open(path, 'r', encoding='utf-8') as file:
                if (os.path.exists(path + '.backup')):
                    ...
                    while True:
                        result = input(
                            "backup already exists, overwrite? [y/N] ").lower()
